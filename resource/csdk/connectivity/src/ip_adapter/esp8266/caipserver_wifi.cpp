...
static const IPAddress IPv4_MULTICAST(224, 0, 1, 187);
...
static void CAESP8266CheckData();
...
// Created two different WiFiUDP instances, one for listening and one for transmitting.
                                  ...
                                  bool secured)
{
    ...
    OIC_LOG_V(DEBUG, TAG, "start unicast @ port: %u", *port);
...
CAResult_t CAIPStartMulticastServer(const char *localAddress, IPAddress multicastAddress,
                                      uint16_t multicastPort)
{
    OIC_LOG(DEBUG, TAG, "IN");
    VERIFY_NON_NULL(&multicastPort, TAG, "port");
    ...
    OIC_LOG_V(DEBUG, TAG, " start multicast @ port: %u", multicastPort);
                              ...
                              const void *data, const uint32_t dataLength)
{
    ...
    {
        CASecureEndpoint_t sep = {.endpoint = {.adapter = CA_ADAPTER_IP, .flags = CA_IPV4, .port = port}};
...
void CAESP8266CheckData()
{
    ...
    if (packetSize)
    {
        ...
        //HACK
        senderPort = 5683;
        OIC_LOG_V(DEBUG, TAG, "****>>>> HACK the new value of port: %d", senderPort);
